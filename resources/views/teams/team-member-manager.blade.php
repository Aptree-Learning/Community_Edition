            ...
            <x-form-section submit="addTeamMember">
                ...
                <x-slot name="form">
                    ...
                    @php
                        ...
                        $notIncludedUsers = $allUsers->filter(function($value, $key) use ($team) {
                            return ! $team->hasUser($value);
                        })
                    @endphp
                    <!-- Member Email -->
                    <div class="col-span-6 sm:col-span-4">
                        <x-label for="email" value="{{ __('Users') }}" />
                        <select class="mt1 block w-full text-sm border border-gray-200 rounded-md" wire:model="addTeamMemberForm.email">
                            @foreach($notIncludedUsers as $eachUser)
                                <option value="{{ $eachUser->email }}">{{ $eachUser->name }} ({{ $eachUser->email }})</option>
                            @endforeach
                        </select>
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Gate::check('addTeamMember', $team) && Laravel\Jetstream\Jetstream::hasRoles())
                                        <button class="ml-2 text-sm text-gray-400 underline" wire:click="manageRole('{{ $user->id }}')">
                                            {{ $user->membership && Laravel\Jetstream\Jetstream::findRole($user->membership->role)->name }}
                                        </button>
